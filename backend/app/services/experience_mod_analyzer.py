...
def analyze_experience_mod(
    ...
    if prior_mod is not None:
        ...
        mod_trend = {
            ...
            "direction": "improving" if mod_change < 0 else ("worsening" if mod_change > 0 else "flat"),
        }
    ...
    # Mod classification
    if current_mod > 1.25:
        flags.append(f"Mod at {current_mod:.2f} — significantly above unity, expect carrier scrutiny")
    elif current_mod > 1.00:
        flags.append(f"Mod at {current_mod:.2f} — above unity, room for improvement")
    elif current_mod < 0.85:
        insights.append(f"Mod at {current_mod:.2f} — well below unity, strong safety record")
    elif current_mod <= 1.00:
        insights.append(f"Mod at {current_mod:.2f} — at or below unity")
    ...
    if mod_claims:
        ...
        for claim in mod_claims:
            ...
            impact = "high" if inc > 50_000 else ("medium" if inc > 15_000 else "low")
            claims_analysis.append({
                **claim,
                "mod_impact": impact,
                "pct_of_total": round(inc / total_claim_incurred, 4) if total_claim_incurred > 0 else 0,
            })
        ...
        # Sort by impact
        claims_analysis.sort(key=lambda c: c.get("incurred", 0) or 0, reverse=True)

        if claims_analysis:
            top = claims_analysis[0]
            if top.get("incurred", 0) > 25_000:
                desc = top.get("description", "largest claim")
                flags.append(
...
def render_experience_mod_text(analysis: dict, account_name: str) -> str:
    """Render experience mod analysis as readable text for the producer."""
    lines = [f"WAYOS PREP — EXPERIENCE MOD WORKSHEET REVIEW: {account_name.upper()}", ""]

    lines.append(f"Current Mod: {analysis['current_mod']:.2f}")
    if analysis.get("prior_mod") is not None:
        lines.append(f"Prior Mod: {analysis['prior_mod']:.2f}")

    trend = analysis.get("mod_trend")
    if trend:
        direction = trend["direction"].upper()
        lines.append(f"Trend: {direction} ({trend['change']:+.2f})")
    lines.append("")
    ...
    # Loss comparison
    la = analysis.get("loss_analysis")
    if la:
        lines.append("EXPECTED vs ACTUAL LOSSES")
        lines.append("")
        lines.append(f"  Expected: ${la['expected_losses']:,.0f}")
        lines.append(f"  Actual Primary: ${la['actual_primary']:,.0f}")
        lines.append(f"  Actual Excess: ${la['actual_excess']:,.0f}")
        lines.append(f"  Total Actual: ${la['actual_total']:,.0f}")
        lines.append(f"  Deviation: ${la['deviation']:,.0f} ({la['deviation_pct']:+.0%})")
        lines.append("")

    # Claims
    ca = analysis.get("claims_analysis", [])
    if ca:
        lines.append("CLAIMS ON WORKSHEET (by impact)")
        lines.append("")
        for c in ca:
            med = " [MED ONLY]" if c.get("medical_only") else ""
            desc = c.get("description", "No description")
            inc = c.get("incurred", 0)
            impact = c.get("mod_impact", "?").upper()
            lines.append(f"  [{impact}] ${inc:,.0f} — {desc}{med}")
        lines.append("")

    # Class codes
    cc = analysis.get("class_code_analysis", [])
    if cc:
        lines.append("CLASS CODES")
        lines.append("")
        for entry in cc:
            desc = entry.get("description") or entry["class_code"]
            payroll = entry.get("payroll", 0)
            lines.append(f"  {entry['class_code']}: {desc} — Payroll ${payroll:,.0f}")
        lines.append("")
